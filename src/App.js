...
const App = () => {
  ...
  const handleSubmit = (event) => {
    ...
    nameExists(newName)
      ? window.confirm(
          `${newName} is already in the phonebook. Do you want to replace the number with the new one`
        ) &&
        personServices
          ...
          .then((returnedPerson) => {
            ...
            notficationHelper(
              `Updated the number of '${returnedPerson.name}' in the phonebook`,
              true
            );
            setNewName("");
            setNewNumber("");
  ...
  const filteredPersons = persons.filter((person) =>
    normalizeName(person.name).includes(normalizeName(nameFilter))
  );
...
const Filter = ({ onChange, value }) => {
  return (
    <p>
      Fitler shown with <input onChange={onChange} value={value} />
    </p>
  );
};
